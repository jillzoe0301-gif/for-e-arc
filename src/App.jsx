  ...
  return (
    <div className="app">
      <aside className="sidebar">
        <div className="logo"><div className="logo-box logo-mark">ARC</div><div><strong>居留證控管</strong><span>Supabase 正式版 V1</span></div></div>
      ...
      <main className="main">
        <div className="topbar">
          <div><h1>{current?.[1] || '總覽'}</h1><p>送件、繳費、對帳、傳真、領件與統計。</p></div>
          <div className="user-chip">目前使用者：<strong>{deviceLock.handler_name}</strong></div>
        </div>
...
function SettingsPage({ data, settings, activeHandlers, activeItems, updateRow, insertRow, loadAll, log, setDeviceLock, deviceId, deviceLock, flash }) {
  ...
  const settingLabels = [
    ['default_fee','手續費預設值'],
    ['admin_unlock_code','管理員解除碼'],
    ['taoyuan_station_phone','桃園移民署電話'],
    ['taoyuan_station_fax','桃園移民署傳真'],
    ['fax_agency_name','傳真表下方仲介公司'],
    ['fax_contact_name','傳真聯絡人'],
    ['fax_contact_phone','傳真聯絡電話']
  ];
  ...
  return <>
    <section className="grid two">
      <div className="card"><div className="section-title"><div><h2>行政 / 承辦選項維護</h2><p className="hint">承辦選項維持精簡排列，不佔版面。</p></div></div><div className="chips">{activeHandlers.map(h=><span className="edit-chip" key={h.id}><input value={h.name} onChange={e=>updateRow('handlers',h.id,{name:e.target.value},'update_handler')} /><button onClick={()=>updateRow('handlers',h.id,{is_active:false},'disable_handler')}>×</button></span>)}</div><div className="toolbar"><input placeholder="新增承辦" value={newHandler} onChange={e=>setNewHandler(e.target.value)} /><button className="primary" onClick={async()=>{ if(newHandler){ await insertRow('handlers',{name:newHandler,display_order:100}); setNewHandler(''); }}}>新增</button></div></div>
      <div className="card"><div className="section-title"><div><h2>系統基本設定</h2><p className="hint">傳真表、手續費與本機鎖定使用。</p></div></div><div className="form-grid">{settingLabels.map(([k,label])=><label key={k}>{label}<input defaultValue={settings[k]||''} onBlur={e=>setSetting(k,e.target.value)} /></label>)}</div></div>
    </section>
    <section className="card"><div className="section-title"><div><h2>申請項目維護</h2><p className="hint">申請項目可新增、修改、停用；初辦與報備類可設定繳費後線上下載。</p></div></div><div className="table-wrap"><table><thead><tr><th>項目</th><th>預設金額</th><th>需領件</th><th>繳後下載</th><th></th></tr></thead><tbody>{activeItems.map(i=><tr key={i.id}><td><input value={i.name} onChange={e=>updateRow('application_items',i.id,{name:e.target.value},'update_item')}/></td><td><input type="number" value={i.default_amount} onChange={e=>updateRow('application_items',i.id,{default_amount:e.target.value},'update_item')}/></td><td><input type="checkbox" checked={i.requires_pickup} onChange={e=>updateRow('application_items',i.id,{requires_pickup:e.target.checked},'update_item')}/></td><td><input type="checkbox" checked={i.download_after_payment} onChange={e=>updateRow('application_items',i.id,{download_after_payment:e.target.checked},'update_item')}/></td><td><button className="danger mini" onClick={()=>updateRow('application_items',i.id,{is_active:false},'disable_item')}>停用</button></td></tr>)}</tbody></table></div><div className="toolbar"><input placeholder="項目名稱" value={newItem.name} onChange={e=>setNewItem({...newItem,name:e.target.value})}/><input type="number" value={newItem.default_amount} onChange={e=>setNewItem({...newItem,default_amount:e.target.value})}/><label className="inline"><input type="checkbox" checked={newItem.requires_pickup} onChange={e=>setNewItem({...newItem,requires_pickup:e.target.checked})}/>需領件</label><label className="inline"><input type="checkbox" checked={newItem.download_after_payment} onChange={e=>setNewItem({...newItem,download_after_payment:e.target.checked})}/>繳後下載</label><button className="primary" onClick={async()=>{ if(newItem.name){ await insertRow('application_items',{...newItem,default_amount:num(newItem.default_amount)}); setNewItem({ name:'', default_amount:1000, requires_pickup:true, download_after_payment:false }); }}}>新增項目</button></div></section>
    <section className="card"><div className="section-title"><div><h2>本機使用者鎖定管理</h2><p className="hint">目前本機使用者：<b>{deviceLock.handler_name}</b>。一般操作不可更換，需管理員解除碼。</p></div></div><div className="toolbar"><input type="password" placeholder="管理員解除碼" value={unlockCode} onChange={e=>setUnlockCode(e.target.value)} /><select value={newHandlerId} onChange={e=>setNewHandlerId(e.target.value)}><option value="">新使用者</option>{activeHandlers.map(h=><option key={h.id} value={h.id}>{h.name}</option>)}</select><input placeholder="變更原因" value={reason} onChange={e=>setReason(e.target.value)} /><button className="primary" onClick={switchUser}>管理員更換使用者</button></div></section>
  </>;
}
