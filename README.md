...
- 批次送件會在儲存前重新讀取畫面資料，不會再因為欄位尚未觸發 onchange 而漏存。
- 若批次中有未完成資料，已完成資料會先新增成功，未完成資料會保留在表格中，不會消失。
- 日期欄位可直接輸入 `20260601`、`2026/06/01` 或 `2026-06-01`，畫面會統一顯示為 `YYYY-MM-DD`。
- 承辦欄位可用下拉選擇，也可從 Excel 垂直整欄複製貼上。
- 仲介別欄位可用下拉選擇，也可從 Excel 垂直整欄複製貼上；可貼「灃康 / 乾坤 / 灃禾」或「FW / WC / FC」。
- 批次送件可同一批登入不同仲介；案件編號會依各列仲介與申請日自動產生。
- 批次表格支援從 Excel 垂直貼上，例如複製 11 位工人姓名後，點第一格工人欄位貼上，系統會自動往下填並新增列。
- Excel 多欄資料也可從指定欄位開始貼上，欄位順序為：承辦、仲介別、雇主、工人、入境日、申請日、申請團號、申請項目、金額.
